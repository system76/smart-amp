...
int tas5825m_setup(struct device *dev, int id)
{
	...
	res = tas5825m_write_at(dev, 0x3B, 0x00);
	if (res < 0)
		return res;
	...
	res = tas5825m_write_at(dev, 0x73, 0x00);
	if (res < 0)
		return res;
...

	{
		const uint8_t values[] = {
			...
			0xF0, 0x73, 0x27, 0x53, 0x07, 0x98, 0xFE
		};
		res = tas5825m_write_block_at(dev, 0x30, values, ARRAY_SIZE(values));
		if (res < 0)
			return res;
	...
	{
		const uint8_t values[] = {
			0x8B, 0x0F, 0x8C, 0xD8, 0xAD, 0xF8, 0x72, 0xD0,
...

	{
		const uint8_t values[] = {
			...
			0xF0, 0x29, 0x79, 0xD3, 0x07, 0xEB, 0x43
		};
		res = tas5825m_write_block_at(dev, 0x08, values, ARRAY_SIZE(values));
		if (res < 0)
			return res;
	...
	{
		const uint8_t values[] = {
			0x17, 0x0F, 0xD6, 0x50, 0x6B, 0xF8, 0x29, 0x44,
			0x10, 0x07, 0xF0, 0xED, 0x8D, 0xF0, 0x42, 0x66,
			0x35, 0x07, 0xCD, 0xBB, 0x6F, 0x0F, 0xBD, 0x99,
			0xCB, 0xF8, 0x41, 0x57, 0x04, 0x08, 0x19
		};
		res = tas5825m_write_block_at(dev, 0x27, values, ARRAY_SIZE(values));
		if (res < 0)
			return res;
	...
	{
		const uint8_t values[] = {
			0xA4, 0x18, 0xF0, 0x0F, 0xF2, 0xD6, 0x07, 0xD7,
			0x7B, 0xA8, 0x0F, 0xF0, 0x60, 0x5B, 0xF8, 0x0F,
			0x33, 0x70, 0x08, 0x00, 0x00, 0x00, 0x00, 0x00,
			0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00
		};
		res = tas5825m_write_block_at(dev, 0x46, values, ARRAY_SIZE(values));
		if (res < 0)
...

	{
		const uint8_t values[] = {
			...
			0x00, 0x08, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
			0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
			0x00, 0x00, 0x00, 0x00, 0x00, 0x07, 0xE3
		};
		res = tas5825m_write_block_at(dev, 0x27, values, ARRAY_SIZE(values));
		if (res < 0)
			return res;
	...
	{
		const uint8_t values[] = {
			0xCF, 0xE3, 0xF0, 0x38, 0x60, 0x3B, 0x07, 0xE3,
			0xCF, 0xE3, 0x0F, 0xC7, 0x6A, 0x36, 0xF8, 0x38,
			0x2A, 0xAB, 0x07, 0xEB, 0x43, 0x17, 0xF0, 0x29,
			0x79, 0xD3, 0x07, 0xEB, 0x43, 0x17, 0x0F
		};
		res = tas5825m_write_block_at(dev, 0x46, values, ARRAY_SIZE(values));
		if (res < 0)
			return res;
	...
	{
		const uint8_t values[] = {
			0xD6, 0x50, 0x6B, 0xF8, 0x29, 0x44, 0x10, 0x07,
			0xF0, 0xED, 0x8D, 0xF0, 0x42, 0x66, 0x35, 0x07,
			0xCD, 0xBB, 0x6F, 0x0F, 0xBD, 0x99, 0xCB, 0xF8,
			0x41, 0x57, 0x04
		};
		res = tas5825m_write_block_at(dev, 0x65, values, ARRAY_SIZE(values));
		if (res < 0)
